...
def build_all_category_pages(input_path: str, template_path: str, output_dir: str) -> None:
    ...
    for product in products:
        if not isinstance(product, dict):
            continue
        normalized = normalize_product(product)
        cat_slug = normalized.get("custom_category_slug", "outros")
        categories.setdefault(cat_slug, []).append(normalized)

        name_lower = (normalized.get("name") or "").lower()
        for brand in ["samsung", "motorola", "lenovo", "lg", "jbl", "apple", "philco", "asus"]:
            if brand in name_lower:
                brands.setdefault(brand, []).append(normalized)
                break
